...
(function (module) {
    ...
    function extractStylesheets(files, html, callback) {
        ...
        stylesheets = doms.map(function (html) {
            ...
            return links
                .filter(function (x) {
                    // Remove stylesheets fetched over http.
                    return links[x].attribs.href.indexOf('http://') !== 0;
                }).map(function (x) {
                    // Get the local href
    ...
    function uncss(files, opt, cb) {
        ...
        if (typeof opt === 'function') {
            // There were no options, so opt is actually the callback.
            ...
            async.waterfall([
                ...
                function (callback) {
                    async.map(
                        files,
                        function (f, callback) {
                            return lib.Phantom(f, options.timeout, callback);
                        },
                        function (res) {
                            if (typeof res !== 'Array') {
                                res = [res];
